...
import { CheckCircle2, Flag, XCircle, Clock, MapPin } from "lucide-react";
...
export default function MechanicVisitRequests() {
  ...
  return (
    <div style={sh.page}>
      ...
      {/* HERO */}
      <div style={sh.hero}>
        <div style={sh.rolePill}><div style={sh.roleDot} /><span style={sh.roleText}><MapPin size={10} style={{marginRight: '2px', verticalAlign: 'middle'}}/> Dispatches</span></div>
      ...
      <div style={sh.content} className="stagger-slide-up">
        ...
        <div style={sh.card}>
          {loading ? (
            ...
            <EmptyState icon={<MapPin size={48} color={colors.warning} />} title="No requests" subtitle="Visit requests assigned to you by owners will appear here." />
          ) : (
            filtered.map((r, i) => (
              <div
                key={r.id}
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "12px", color: colors.textSecondary }}><MapPin size={12} style={{display:'inline-block', verticalAlign:'middle'}}/> {r.address || "No address"}</div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 100, display: "flex", alignItems: "flex-end", animation: "ab-fade-in 0.2s ease-out" }} onClick={() => setSelected(null)}>
          <div style={{ background: colors.white, borderRadius: "24px 24px 0 0", width: "100%", padding: "1.5rem 1.25rem", maxHeight: "90vh", overflowY: "auto", animation: "ab-slide-up 0.35s cubic-bezier(0.16,1,0.3,1) forwards" }} onClick={e => e.stopPropagation()}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
              <div style={{ fontWeight: "800", fontSize: "17px", color: colors.navy, display: "flex", alignItems: "center", gap: "4px" }}><MapPin size={18} /> Visit Request</div>
            ...
            <div style={{ marginTop: "1.5rem", display: "flex", flexDirection: "column", gap: "10px" }}>
              {["accepted", "pending"].includes((selected.status || "").toLowerCase()) && (
                <button
                  ...
                  disabled={saving}
                  style={{ width: "100%", padding: "14px", background: `linear-gradient(135deg, ${colors.navy}, ${colors.blue})`, color: "#fff", fontSize: "14px", fontWeight: "700", border: "none", borderRadius: "14px", cursor: "pointer", fontFamily: "inherit", boxShadow: "0 4px 12px rgba(26,58,92,0.25)", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}
                >
                  {saving ? "Saving..." : <><Flag size={16}/> Mark as Completed</>}
                </button>
              )}
              {selected.status === "Completed" && (
                <div style={{ fontSize: "13px", color: colors.success, fontWeight: "600", padding: "12px", background: colors.successBg, borderRadius: "12px", border: `1px solid rgba(22,163,74,0.3)`, display: "flex", alignItems: "center", gap: "6px" }}>
                  <CheckCircle2 size={16}/> This visit has been completed.
                </div>
              )}
              {["declined", "cancelled"].includes((selected.status || "").toLowerCase()) && (
                <div style={{ fontSize: "13px", color: colors.danger, fontWeight: "600", padding: "12px", background: colors.dangerBg, borderRadius: "12px", display: "flex", alignItems: "center", gap: "6px" }}>
