                        ...
                        {editingCommentId === comment.id ? (
                          <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginTop: "4px" }}>
                            ...
                            <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
                              ...
                              <button onClick={() => handleEditCommentSubmit(comment.id, activeCommentPost.id)} disabled={!editCommentText.trim() || editCommentText.trim() === comment.text.trim()} style={{ background: (editCommentText.trim() && editCommentText.trim() !== comment.text.trim()) ? colors.navy : colors.bg, color: (editCommentText.trim() && editCommentText.trim() !== comment.text.trim()) ? "#fff" : colors.textMuted, border: "none", borderRadius: "6px", padding: "4px 12px", fontSize: "12px", fontWeight: "700", cursor: (editCommentText.trim() && editCommentText.trim() !== comment.text.trim()) ? "pointer" : "not-allowed", transition: "background 0.2s, color 0.2s" }}>Save</button>
                            </div>
                          </div>
                        ) : (
                          <>
